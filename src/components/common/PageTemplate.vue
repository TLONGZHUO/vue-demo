<template>
  <div class="pageTemplate">
    ...
    <span v-if="pageNo !== 1" :class="myCurrent" @click="goPage(1)">1</span>
    <span v-if="preClipped" class="page-index">...</span>
    <span v-for="index in pages" :class="myCurrent" @click="goPage(index)">{{index}}</span>
    <span v-if="backClipped" class="page-index">...</span>
    <span :class=myPageNo @click="goPage(pageNo)">{{pageNo}}</span>
    <button :class=myPageNo @click="nextPage">下一页</button>
  </div>
</template>
<script>
  export default {
    ...
    props: {
      ...
      myCurrent: 'page-index '+ 1 == this.current ? 'active':'',
      myPageNo: 'page-index '+ this.pageNo == this.current ? 'active':''
    },
    data: function () {
      return {
